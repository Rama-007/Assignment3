App = {
    ...
    try{
      ...
      // console.log(out);
        ...
        while(true)
        {
          try
          {
            var bokka1 = await Instance.transfer_mp.call(i);
            console.log(bokka1);
            console.log(bokka1[0],bokka1[1]['c'][0],bokka1[2]['c'][0],bokka1[3]['c'][0]);
            temp.push([bokka1[1]['c'][0],bokka1[2]['c'][0]]);
            // var bokka=bokka1.toNumber();
            // temp.push(bokka1.toNumber());
            i++;
          }
        ...
        for(i=0;i<temp.length;i++)
        {
          if(temp[i][0]!=0 && temp[i][1]!=0)
          {
            // $("#trade").append("<tr>")
            // $("#trade").append("<td>")
            App.display_pokemon_id($("#trade"),temp[i][0]-1);
            // $("#trade").append("</td>")
            // $("#trade").append("<td>")
            App.display_pokemon_id($("#trade"),temp[i][1]-1);
            // $("#trade").append("</td>")
            // $("#trade").append("</tr>")
            $("#trade").append("<br/>")
          }
  ...
  sendTrade: async function()
  {
    var Instance = await App.contracts.Uber.deployed();
    var send = document.getElementById("start").value;
    var want = document.getElementById("stop").value;

    try{
      var temp = await Instance.put_in_transfer(send,want,{from:App.account});
      console.log("hie");
    }
    catch(err)
    {
  ...
  Trade_return: async function(){
    var Instance = await App.contracts.Uber.deployed();
    try{
      var temp = await Instance.transfer_not_happened({from:App.account});
    }
    catch(err)
    {
      console.log(err);
...
$(function() {
  ...
  var rett=setInterval(App.Trade_return,60000);
